...
public:
    DataManager() : Node("data_manager")
    {
        ...
        steer_subscription_ = this->create_subscription<std_msgs::msg::Char>(
        ...
        steer_publisher_ = this->create_publisher<std_msgs::msg::Char>("control/steer/new", 10);
    }
private:
    ...
    void steer_callback(const std_msgs::msg::Char::SharedPtr msg) {
        auto new_message = std_msgs::msg::Char();
        new_message.data = msg->data;
        switch (mode_) {
            case trike::PARK:
    ...
    void mode_callback(const std_msgs::msg::Int8::SharedPtr msg) {
        mode_ = msg->data;
        auto new_brake_message = std_msgs::msg::Int8();
        auto new_steer_message = std_msgs::msg::Char();
        if(mode_ == trike::PARK) {
            // Publish 1 when in PARK mode
            new_brake_message.data = 1;
            brake_publisher_->publish(new_brake_message);
        } else if(mode_ == trike::NEUTRAL) {
            // Publish 0 when in NEUTRAL mode
            new_brake_message.data = 0;
            brake_publisher_->publish(new_brake_message);
        }

        new_steer_message.data = 'k';
        steer_publisher_->publish(new_steer_message);
    }

    // Subscriptions
    rclcpp::Subscription<std_msgs::msg::Int8>::SharedPtr brake_subscription_;
    rclcpp::Subscription<std_msgs::msg::Char>::SharedPtr steer_subscription_;
    ...
    rclcpp::Publisher<std_msgs::msg::Char>::SharedPtr steer_publisher_;
